...
class CollabFilterOneScalarPerItem(AbstractBaseCollabFilterSGD):
    ...
    def predict(self, user_id_N, item_id_N,
        ...
        **param_dict : unpacked parameter dict
        ...
        N = user_id_N.size
        #yhat_N = ag_np.full(1, mu[0] + b_per_user[user_id_N[0]] + c_per_item[item_id_N[0]])
        #for i in range(1, len(user_id_N)):
        #    yhat_N = ag_np.hstack((yhat_N, ag_np.full(1, mu[0] + b_per_user[user_id_N[i]] + c_per_item[item_id_N[i]])))
        #yhat_N = ag_np.ones(N)
        if b_per_user is None:
            b_per_user = self.param_dict['b_per_user']
        if c_per_item is None:
            c_per_item = self.param_dict['c_per_item']
        yhat_N = mu[0] + b_per_user[user_id_N] + c_per_item[item_id_N]
        return yhat_N
